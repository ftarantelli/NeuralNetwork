...
dvec dec_discr_Chebys(const int N, const dvec xx, const dvec yy) {
...
dvec dec_cont_Chebys(const int N, const double a, const double b, const auto integ_func) {
    //int N = 50; // Grado massimo della scomposizione
    //double a(0.), b(1.); // Estremi dell'intervallo
    double integral(0.), error(0.);
	const int max_iterations = 15;
    ...
    for (int n = 0; n <= N; ++n) {
		...
		auto function = [n,a,b](double xvar) {
			double yvar = 2./(b-a)*(xvar - a/2. - b/2.);
			return chebyshev_t(n, yvar) / sqrt(1.0 - std::pow(yvar,2.)) * 2./(b-a) *integ_func(xvar);
		};
